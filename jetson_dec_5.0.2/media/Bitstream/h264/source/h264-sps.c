...
void h264_sps_release(struct h264_sps_t* sps){
	if(sps!=NULL&&1 == sps->pic_order_cnt_type){
...
int h264_sps(bitstream_t* stream, struct h264_sps_t* sps)
{
	...
	*/
	...
	else if(1 == sps->pic_order_cnt_type)
	{
		...
		sps->offset_for_ref_frame = (int32_t*)malloc(sps->num_ref_frames_in_pic_order_cnt_cycle * sizeof(int32_t));//need to free
